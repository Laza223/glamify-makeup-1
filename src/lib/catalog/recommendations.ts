...
export async function getOrderBumpOffers(): Promise<BumpOffer[]> {
  ...
  if (taggedOffers.length > 0) return taggedOffers;

  // Fallback garantizado: si ningún producto tiene el tag "order-bump", ofrecer los productos más accesibles con stock

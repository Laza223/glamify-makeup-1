...
- [x] **Reseñas (moderación):** aprobar/rechazar `Review` desde el panel (`/admin/resenas`).
- [x] **Ajustes / Settings page:** editar `Setting` (storeName, envío gratis, CP origen, redes) desde el panel (`/admin/ajustes`).
- [ ] **ShippingZone CRUD:** alta/edición de zonas de envío desde el panel (hoy solo por seed/DB).
...
- [~] **Duplicar producto** — descartado por decisión de producto en v1.
...
- [x] **Fotos en reseñas** — subida a Supabase Storage con previsualización, zoom en storefront y preview en moderación de `/admin/resenas`.
...
- [x] **CRUD admin de solicitudes de arrepentimiento** — vista en `/admin/arrepentimiento` para listar con ticket `ARR-NNNNNN`, contactar por WhatsApp/email directo y cambiar estado (`pending`/`processed`/`rejected`).
- [~] **Formulario de contacto** — descartado por decisión de producto en v1 (atención 100% canalizada por WhatsApp directo y email).
...
- [x] **Cotización en vivo por CP** — resuelto con la **API oficial de MiCorreo** (`lib/shipping/micorreo.ts`, camino self-service sin acuerdo comercial; ver `docs/decisions/0001-shipping-provider.md`). Zipnova cancelado por markup ~2x (`lib/shipping/zipnova.ts` queda huérfano, sin borrar). Falta cargar los secrets `MICORREO_EMAIL/PASSWORD/GATEWAY_AUTH` en el entorno y verificar con `pnpm micorreo:probe`; sin ellos cae a la tabla de zonas (ya recalibrada al costo real).
